...
function code_block(node) {
  ...
  // fenced code block
  if(node._isFenced || node.info) {
    if(node._fenceChar && node._fenceLength) {
      chars = repeat(node._fenceChar, node._fenceLength)
    }

    // ensure that when the literal does not have a trailing
    // newline we terminate with a newline so the end of the fenced
    // code block is not attached to the literal which causes a 
    // rendering error
    if(!/\r?\n$/.test(literal)) {
      literal += EOL; 
    }
    ...
    this.out(
      chars + info + this.newline + literal + chars);
  // indented code block
  }else{
    chars = repeat(' ', 4);
    lines = node.literal.split('\n');
    for(var i = 0;i < lines.length;i++) {
      this.out(chars + lines[i]);
      if(i < lines.length - 1) {
        this.cr(); 
      }
    }
...
function list(node, entering) {
  if(!entering) {
    ...
    if(!info.parents.length
      && (!node.next && (node.parent && !node.parent.next))) {
      // trailing blank line at end of document
      if(Node.is(node.parent, Node.DOCUMENT)
        && (node._lastLineBlank || node.lastLineBlank)) {
        this.cr(); 
      }
      return; 
    }

    if(node._lastLineBlank || node.lastLineBlank
      // NOTE: must also check last item in list
      || (node.lastChild && node.lastChild._lastLineBlank)) {
      this.cr(); 
    }
  }
}
...
function item(node, entering) {
  var info = getListParents(node)
    , parents = info.parents.concat(info.current)
    , padding = 0
    , indent;
