 ...
 *  @option {String=1} [section] man page section number.
 *  @option {String=UNTITLED} [title] title for the man page.
 *  @option {String=1.0} [preambleVersion] version for the preamble.
 *  @option {String} [description] description for the preamble.
 *  @option {String} [locale] locale used for date formatting.
 *  @option {String=emph|strong} [inline] formatting for inline code.
 *  @option {Boolean=false} [html] force inclusion of HTML blocks.
 */
function ManRenderer(opts) {
  ...
  // keep track of current heading depth for indentation
  this.depth = 0;
}
...
function softbreak(node/*, entering*/) {
  ...
  // handle adding a line break on the first text node
  // when in an options list item (declared with +) which uses .TP
  if(grandparent
    && Node.is(grandparent, Node.ITEM)
    && Node.is(node.parent, Node.PARAGRAPH)
    && node.parent.firstChild !== node
    && grandparent._listData.bulletChar === '+'
    && !grandparent._listData.lineBroken) {
      this.out('\n');
      grandparent._listData.lineBroken = true;
      return;
  }
...
function paragraph(node, entering) {
  ...
  if(entering) {
    ...
    if(node.parent 
      && Node.is(node.parent, Node.ITEM)
      && node.parent.firstChild === node) {
      return; 
    }

    // catches subsequent paragraphs in options declarations
    // ie those using the '+' list bullet
    if(node.parent
      && Node.is(node.parent, Node.ITEM)
      && node.parent._listData.bulletChar === '+') {
      this.out('\n.TP\n.in 8\n');
    }else if(!node.parent
  ...
  if(entering) {
    if(bullet !== '+') {
      ...
      if(node.firstChild && node.firstChild.literal) {
        if(/^ /.test(node.firstChild.literal)) {
          this.out(' .in 7'); 
        } 
      }
      this.out('\n');
